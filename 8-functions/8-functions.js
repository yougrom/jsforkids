...
// Первый аргумент сообщает функции сколько раз это нужно печатать, второй аргумент сообщает функции что печатать
...
let randomBodyPartLenght =
  randomBodyParts[Math.floor(Math.random() * randomBodyParts.length)];
let randomAdjectiveLenght =
  randomAdjectives[Math.floor(Math.random() * randomAdjectives.length)];
let randomNewWordLenght =
  randomNewWords[Math.floor(Math.random() * randomNewWords.length)];
...
let randomInsultLenght = [
  "У тебя",
  randomBodyPartLenght,
  "словно",
  randomAdjectiveLenght,
  randomNewWordLenght,
  "!!!",
].join(" ");
